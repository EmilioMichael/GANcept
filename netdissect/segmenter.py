...
class UnifiedParsingSegmenter(BaseSegmenter):
    ...
    def __init__(self, segmodel_path='', segsizes=None, segdiv=None):
        ...
        if segmodel_path == '':
            ensure_upp_segmenter_downloaded('/content/upp-resnet50-upernet')
            segmodel = load_unified_parsing_segmentation_model('/content/upp-resnet50-upernet/')
        else:
            segmodel = load_unified_parsing_segmentation_model('/content/upp-resnet50-upernet/')
...
def load_unified_parsing_segmentation_model(segmodel_path, segmodel_arch=('resnet101', 'upernet'), segvocab='upp', epoch=40):
    # segmodel_dir = 'dataset/segmodel/%s-%s-%s' % ((segvocab,) + segmodel_arch)
    segmodel_dir = segmodel_path
